...
from src.epicorAPI.CSIProducts import get_csi_sales, categories, material_codes, competitors, CSIPart
...
def get_csi_products_df(df: pl.DataFrame):
    rows = df.filter(
        pl.col("partnum").str.starts_with("CSI-") | pl.col("partnum").str.split("-").list.get(0).is_in(
            categories)
...
def get_csi_category_monthly_counts(df: pl.DataFrame):
    return (
        ...
        df
        .with_columns(
            pl.col("changedate").str.strptime(pl.Datetime, strict=False)  # convert str → datetime
        )
        .with_columns(
        ...
        .agg(
            pl.col("cost").count().alias("totalCount")
        )
        .with_columns(
            (
                    pl.col("totalCount")
                    / pl.col("totalCount").sum().over("yearMonth") * 100
            ).alias("percentage")
        )
...
def get_csi_category_monthly_revenues(df: pl.DataFrame):
    return (
        ...
        .group_by(["yearMonth", "categoryid"])
        .agg(
            pl.col("cost").sum().alias("totalRevenue")
        )
        .with_columns(
            (
                    pl.col("totalRevenue")
                    / pl.col("totalRevenue").sum().over("yearMonth") * 100
            ).alias("percentage")
        )
        .sort(["yearMonth", "categoryid"])
...
def csi_category_overview_analysis(df: pl.DataFrame):
    csi_products_df = get_csi_products_df(df=df).filter(pl.col("categoryid").is_in([
        'BFL',
        'CLD',
        'GRD',
        'SUR',
    ...
    result['overview']['categoryMonthlyCountsMonth'] = to_json_grouped_by_month(csi_category_montly_counts)
    result['overview']['categoryMonthlyRevenuesMonth'] = to_json_grouped_by_month(csi_category_montly_revenue)
    result['overview']['categoryMonthlyCountsCategory'] = to_json_grouped_by_category(csi_category_montly_counts)
    result['overview']['categoryMonthlyRevenuesCategory'] = to_json_grouped_by_category(csi_category_montly_revenue)
